...
def daily_line_summary():
    ...
    try:
        ...
        push_text = safe_generate(condensed_prompt).strip()
...
def morning_meditation_job():
    ...
    try:
        ...
        # ── 6. 呼叫 Gemini (精簡版 Prompt) ──
        prompt = (
            "你現在是我的專屬靈感教練與生活督導『小助手』。你深諳家醫科臨床、公共衛生、AI 系統架構，同時也是一位犀利卻溫暖的婚姻/教養顧問與文史哲學者。\n\n"
            "【重要】請根據以下我的 Persona 檔案調整你的語氣，使之更貼近我當前的心境與痛點。\n\n"
            "請閱讀以下我提供的【個人 Persona】、【近期狀態】、【歷史的迴音】、【近期文章存檔】，以及抽出的【跨界筆記】。\n"
...
def process_audio_transcription(message_id, reply_token):
    ...
    try:
        ...
        # 2. MLX Whisper 精準轉錄
        print(f"[語音] 開始轉錄 (large-v3-turbo)...")
        # 關鍵要求：加入 initial_prompt 處理醫療術語
        initial_prompt = "這是一段台灣家醫科醫師的語音紀錄，包含高血壓、糖尿病、HbA1c、SOAP 筆記等醫療專業術語，請以繁體中文輸出。"
        ...
        print(f"[語音] 轉錄完成：{raw_text[:100]}...")

        if not raw_text:
            send_push_message(AUTHORIZED_USER_ID, "老闆，這段語音好像沒有聲音，我聽不清楚喔！")
            return

        # 3. Gemini 2.5 Flash 智慧後處理
        print(f"[語音] 呼叫 Gemini 2.5 Flash 進行精準校對...")
        correction_prompt = (
            "你是一個極度精準的語音轉錄校對員。你的唯一任務是接收語音辨識產出的逐字稿，並進行『極輕量』的文字清理。\n"
            "絕對遵守以下 4 大鐵律：\n\n"
            "【100% 忠於原意與長度】：使用者講多長，你就輸出多長。絕對禁止擴充句子、總結重點、或自行腦補任何沒有提到的資訊。\n"
            "【只做錯字與標點修正】：僅修正同音錯字、修飾不通順的贅字（如「那個」、「然後」），並加上正確的標點符號。確保醫療專業術語拼寫正確。\n"
            "【零排版、零標籤】：絕對不要加上任何 Markdown 標題 (##)、分隔線 (---)、YAML 屬性 (tags, date)、列表符號 (*)。\n"
            "【禁止任何對話回應】：絕對不要輸出任何開場白或結語（例如：「好的」、「這是為您整理的」、「以下是...\"）。直接輸出校對後的純文字內容。\n\n"
            f"【語音逐字稿內容】\n{raw_text}"
        )
        ...
        # 5. 回覆使用者校對結果
        reply_msg = f"🎙️ 語音解析完成：\n\n{corrected_text}"
        send_push_message(AUTHORIZED_USER_ID, reply_msg)
